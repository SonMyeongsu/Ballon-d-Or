...
public class SecurityConfig {
	...
	public SecurityFilterChain filterChain(HttpSecurity security) throws Exception{	//메소드 실행 때 예외처리 다른 곳에서 할게 (다른곳으로 던져 버리겠다)
		...
		security.csrf().disable() /* disable(): 태그의 disabled 속성은 해당 태그 요소가 비활성화됨을 명시함. */ 
				...
				//로그인 회원가입 게시글 목록
				.antMatchers("/member/loginPage" /* antMatchers() : 특정 리소스에 대해서 권한을 설정합니다. */
			...
			.and()
				...
				.defaultSuccessUrl("/board/list") // defaultSuccessUrl() : 정상적으로 인증성공 했을 경우 이동 페이지를 설정함.
			...
			.and()
				...
				.logoutSuccessUrl("/board/list"); // logoutSuccessUrl() : 로그아웃 성공시 리다이렉트 주소.
